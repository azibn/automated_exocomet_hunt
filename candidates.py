...
parser.add_argument(
    "-p", help="saves candidates to csv and saves plots of targets", dest="f"
)
parser.add_argument(
    "-s", help="saves candidates to csv only", default=f"candidates.csv", dest="s"
)
...
def region_of_interest(
    candidates,
    min_asym=args.min_asym,
    max_asym=args.max_asym,
    min_snr=args.min_snr,
    max_snr=args.max_snr,
):
    candidates_in_region = candidates[
        (candidates.asym_score >= min_asym)
        & (candidates.asym_score <= max_asym)
        & (candidates["signal/noise"] >= min_snr)
        & (candidates["signal/noise"] <= max_snr)
    ]
...
def distribution_plot(data):
    ...
    rect = patches.Rectangle(
        (1.05, 5), 2, 30, linewidth=3, edgecolor="r", facecolor="none"
    )
    plt.show()
