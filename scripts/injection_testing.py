...
parser.add_argument(
    "-use_noiseless",
    dest="use_noiseless",
    help="convert lightcurves to noiseless ones",
    action="store_true",
)
parser.add_argument(
    "-drop_bad_points",
    dest="drop_bad_points",
    help="cut out flagged data points",
    action="store_true",
)
parser.add_argument(
    "-method",
    dest="method",
    help="select smoothing method. Default is median",
    default="median",
)
parser.add_argument(
    ...
    default=100,
    type=int,
    help='specify the percentage change in depth for a "successful" recovery. Default is 100%',
)
...
def run_injection(
    ...
    use_noiseless_lightcurves=args.use_noiseless,
    drop_bad_points=args.drop_bad_points,
):
    ...
    for i in lc_paths:
        ...
        if use_noiseless_lightcurves:
            if not drop_bad_points:
                ...
                _, injected_time = random.choice(list(enumerate(time_range)))
                comet = 1 - comet_curve(
                    time, depth, injected_time, 3.02715600e-01, 3.40346173e-01
                )
                ...
                if args.method == None:
                    data["injected_dip_flux"] = (
                        np.ones(len(data["corrected flux"])) * comet
                    ) - 1
                else:
                    data["injected_dip_flux"] = (
                        np.ones(len(data["corrected flux"])) * comet
                    ) * np.median(data["corrected flux"])

        ## converted to astropy table
        data_to_process = Table.from_pandas(data)
        data_to_process = data_to_process[
            ["time", "injected_dip_flux", "quality", "flux error"]
        ]

        ## the meat of the search is done by this function
        results, data_arrays = processing(
            data_to_process,
            i,
            lc_info,
            method=args.method,
            noiseless=use_noiseless_lightcurves,
        )
    ...
    if args.save_csv:
        ...
        try:
            os.makedirs(f"injection_recovery_{args.percentage_threshold}percent_8day")
            print(
                f"created directory injection_{args.percentage_threshold}percent_8day"
            )
        except FileExistsError:
            pass
        ...
        try:
            os.makedirs(
                f"injection_recovery_{args.percentage_threshold}percent_8day/sector_{args.sector[0]}"
            )
            print(
                f"created directory injection_{args.percentage_threshold}percent_8day/sector_{args.sector[0]}"
            )
        except FileExistsError:
        ...
        try:
            df.to_csv(
                f"injection_recovery_{args.percentage_threshold}percent_8day/sector_{args.sector[0]}/tmag_{args.mag_lower}_tmag_{args.mag_higher}.csv"
            )
    ...
    for path in paths:
        ...
        if not list(folders_in(path)):
            ...
            try:
                ...
                lookup = pd.read_csv(
                    f"/storage/astro2/phrdhx/tesslcs/sector{args.sector}lookupv2.csv"
                )
                run_injection(path)
