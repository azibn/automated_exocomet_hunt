...
class SimpleSOMMapper(Mapper):
    ...
    def __init__(
        ...
        self._dqdshape = np.array(
            [
                self.kshape[0] / 2,
                self.kshape[1] / 2,
                np.ceil(self.kshape[0] / 2.0).astype("int"),
                np.ceil(self.kshape[1] / 2.0).astype("int"),
            ]
        )
    ...
    def _pretrain(self, samples):
        ...
        # components
        if ifunc is None:
            ifunc = lambda x: np.random.standard_normal(
                tuple(self.kshape) + (x.shape[1],)
            )
        ...
        self._dqd = np.fromfunction(
            self.distance_metric, self._dqdshape[2:] + 1, dtype="float"
        )
    ...
    def _access_kohonen(self):
        ...
        if self._K is None:
            raise RuntimeError(
                "The SOM needs to be trained before access to the Kohonen layer is possible."
            )
