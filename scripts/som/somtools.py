...
def Proportions(klayer, mapped, groups, ngroups, binsx, binsy):
    ...
    for groupidx in range(ngroups):
        map = groups == groupidx
        hists.append(
            np.histogram2d(mapped[map][:, 0], mapped[map][:, 1], bins=[binsx, binsy])[0]
        )
    ...
    for obj in range(map_all.shape[0]):
        ...
        for classidx in range(ngroups):
            for iteridx in range(map_all.shape[2]):
                localprop = proportions[classidx][
                    map_all[obj, 0, iteridx], map_all[obj, 1, iteridx]
                ]
                localweight = proportion_weights[
                    map_all[obj, 0, iteridx], map_all[obj, 1, iteridx]
                ]
                if localprop >= 0:
...
def Classify_Distances(map_all, avgdistances):
    ...
    for obj in range(map_all.shape[0]):
        ...
        for iteridx in range(map_all.shape[2]):
            class_power[obj, 0] += som_power_fp[
                map_all[obj, 0, iteridx], map_all[obj, 1, iteridx]
            ]
            class_power[obj, 1] += som_power_planet[
                map_all[obj, 0, iteridx], map_all[obj, 1, iteridx]
            ]
...
def PixelClassifier(
    klayer, SOMarray, grouparray, ngroups, normalise=False, lowbound=0, highbound=1000
):
    ...
    for xpixel in range(klayer.shape[0]):
        ...
        for ypixel in range(klayer.shape[1]):
            ...
            for dataindex in range(SOMarray.shape[0]):
                distances[dataindex] = ChiSqDist(
                    klayer[xpixel, ypixel, lowbound:highbound],
                    SOMarray[dataindex, lowbound:highbound],
                )
            for groupindex in range(ngroups):
                avgdistances[xpixel, ypixel, groupindex] = np.mean(
                    distances[grouparray == groupindex]
                )
    if normalise:
        for groupindex in range(ngroups):
            maxdist = np.sort(avgdistances[:, :, groupindex].flatten())[
                -normalise
            ]  # ignores the normalise largest distances. These can be anomalies - the key area we want is larger. 5 is a good value for K2
            mindist = np.min(avgdistances[:, :, groupindex])
            avgdistances[:, :, groupindex] = (
                1.0 / (maxdist - mindist) * (avgdistances[:, :, groupindex] - mindist)
            )
    return avgdistances
