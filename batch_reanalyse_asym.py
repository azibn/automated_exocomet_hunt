...
parser = argparse.ArgumentParser(
    description="Analyse lightcurves listed in output file."
)
...
def get_one(file, path=args.d):
    ...
    if not os.path.exists(file_path + file):
        subprocess.run(
            [
                "wget",
                "https://archive.stsci.edu/missions/kepler/lightcurves/{}/{}/{}".format(
                    kic_part, kic_no, file
                ),
                "-O",
                path + "/data/lightcurves/{}/{}".format(kic_no, file),
                "-nc",
            ],
            check=True,
